...
build_plot <- function(ep, vi, mi, di) {
  ...
  ggplot() +
    ...
    scale_x_continuous(
      name     = "Days from Baseline (Day 0 / DM.RFXSTDTC)",
    ...
    labs(
      title    = "Swimmer Plot — Disposition (DS), Study Visits (SV) and Study Elements (SE) by Subject",
      subtitle = paste0(
        "Dashed line = Day 0 (Baseline - VISITNUM=3)\n  ",
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      div(
        ...
        br(),
        icon("info-circle"),
        "Put the cursor inside bars to see more information",
      ),
  ...
  br(),
      ...
      selectInput(
        inputId  = "site",
        label    = "Site Selection (DM.SITEID):",
      ...
      selectInput(
        inputId  = "subjects",
        label    = "Select Specific Subject no (USUBJID):",
        choices  = sort(unique(dm$USUBJID)),
        selected = sort(unique(dm$USUBJID)),
        multiple = TRUE
      )
